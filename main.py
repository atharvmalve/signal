...
def extract_student_history_assets(profile: Dict[str, Any]) -> List[str]:
    """
    Parses core textual historical contexts to isolate specific and discrete 
    technical, leadership, domain, and personal background assets.
    """
    assets = set()
    
    mapping = {
        "3d cnn": "3D CNN forecasting",
        "cnn": "computer vision modeling",
        "gnn": "graph neural networks",
        "lstm": "time-series modeling",
        "transformer": "natural language transformers",
        "robotics": "robotics builder",
        "drone": "autonomous systems engineering",
        "hardware": "hardware prototyping",
        "lidar": "lidar mapping",
        "arduino": "arduino programming",
        "satellite": "satellite systems engineering",
        "quant": "quantitative finance research",
        "trading": "financial markets analysis",
        "finance": "financial systems architecture",
        "economics club": "economics club leadership",
        "math team": "advanced mathematical modeling",
        "healthcare ml": "healthcare ML pipelines",
        "pneumonia": "pneumonia detection modeling",
        "medical": "medical data analytics",
        "climate": "climate science research",
        "environment": "environmental sensing infrastructure",
        "game modding": "game modding environment loops",
        "debate": "speech and debate communications",
        "speech": "speech and debate communications",
    ...
    fields_to_analyze = [
        "club_information", "toughest_project", "future_goal", 
        "major", "program_goals", "existing_project_idea"
    ]
    
    for field in fields_to_analyze:
        text_val = str(profile.get(field, "")).lower()
        for check_key, asset_tag in mapping.items():
            if check_key in text_val:
                assets.add(asset_tag)
                
    if not assets:
        major_clean = str(profile.get("major", "Computer Science")).lower()
        assets.add(f"{major_clean} processing systems")
...
def infer_student_archetype(profile: Dict[str, Any]) -> str:
    ...
    idea = str(profile.get("existing_project_idea", "")).lower()
    
    combined_context = f"{goal} {major} {toughest} {idea}"
    
    if "quant" in combined_context or "high-frequency" in combined_context or "trading" in combined_context:
        return "Quant Researcher"
    if "hospital" in combined_context or "medical" in combined_context or "healthcare" in combined_context or "biomedical" in combined_context or "disease" in combined_context:
        return "Healthcare Innovator"
    if "climate" in combined_context or "environment" in combined_context or "carbon" in combined_context or "green" in combined_context or "pollution" in combined_context:
        return "Climate Scientist"
    if "robot" in combined_context or "hardware" in combined_context or "robotics" in combined_context or "drone" in combined_context or "sensor" in combined_context:
        return "Robotics Builder"
    if "policy" in combined_context or "law" in combined_context or "government" in combined_context or "ethics" in combined_context:
        return "Policy Thinker"
    if "found" in combined_context or "startup" in combined_context or "entrepreneur" in combined_context or "saas" in combined_context or "business" in combined_context:
        return "Founder"
    if "research" in combined_context or "phd" in combined_context or "academic" in combined_context or "paper" in combined_context:
        return "Researcher"
    if "teach" in combined_context or "education" in combined_context or "learn" in combined_context or "school" in combined_context:
        return "Education Builder"
    if "social" in combined_context or "good" in combined_context or "community" in combined_context or "ngo" in combined_context:
...
def execute_hybrid_retrieval(query_context: str, student_info: Dict[str, Any], top_k: int = 2) -> List[Dict[str, Any]]:
    ...
    for idx, row in PROJECTS_DF.iterrows():
        ...
        row_desc = str(row.get("Description", "")).lower().strip()
        row_title = str(row.get("Title", "")).lower().strip()
        combined_row_text = f"{row_cat} {row_sector} {row_desc} {row_title}"
        
        # Step 1: Category Matching Matrix (0.45 Weight)
        category_match = 0.0
        ...
        # Step 2: Keyword & Asset Intersection (0.35 Weight)
        row_tags = PROJECT_TAGS_MAP.get(idx, set())
        tag_overlap_count = len(opportunity_tags.intersection(row_tags))
        
        asset_overlap_count = 0
        for asset in history_assets:
            if asset in combined_row_text:
                asset_overlap_count += 1
                
        total_overlap = tag_overlap_count + asset_overlap_count
        tag_overlap_score = min(1.0, total_overlap / 5.0) if total_overlap > 0 else 0.0
        
        # Step 3: Semantic Similarity Verification (0.20 Weight)
        sem_score = max(0.0, min(1.0, float(semantic_sims[idx])))
        
        final_score = (0.45 * category_match) + (0.35 * tag_overlap_score) + (0.20 * sem_score)
...
def resolve_and_score_profile(inp: ParticipantInput) -> Dict[str, Any]:
    """
    Resolves participant fields against cached Tally forms. Extends fields elegantly 
    for custom/manually entered students without altering incoming payload schemas.
    """
    ...
    resolved = {
        ...
        "future_goal": tally.get("future_goal", ""),
        ...
        "club_information": tally.get("club_information", ""),
        "program_goals": tally.get("program_goals", ""),
    ...
    if not tally:
        if inp.interest_areas:
            manual_themes = [t.strip() for t in inp.interest_areas.split(",") if t.strip()]
            resolved["project_themes"] = [t for t in manual_themes if t in VALID_THEMES]
        
        resolved["toughest_project"] = resolved["toughest_project"] or f"Advanced engineering build featuring {inp.interest_areas or 'applied machine learning software tools'}"
        resolved["future_goal"] = resolved["future_goal"] or f"Research and system deployment inside the {inp.interest_areas or 'computational sciences'} industry"
        resolved["club_information"] = resolved["club_information"] or "Founder of regional computing group and local engineering builder circles"
        resolved["program_goals"] = resolved["program_goals"] or "Scale non-trivial predictive pipelines targeting production-grade real-world deployment challenges"

    if inp.interest_areas:
        manual = [t.strip() for t in inp.interest_areas.split(",") if t.strip()]
        resolved["project_themes"] = list(set([t for t in manual if t in VALID_THEMES] + resolved["project_themes"]))
        
    resolved["archetype"] = infer_student_archetype(resolved)
    resolved["history_assets"] = extract_student_history_assets(resolved)
    return resolved

def compile_identity_graph(profiles: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Synthesizes group details and explicitly generates unique aggregated group history assets.
    """
    ...
    all_history_assets = []
    for p in profiles:
        all_history_assets.extend(p.get("history_assets", []))
    group_history_assets = list(set(all_history_assets))
    ...
    group_matrix_breakdown = []
    for p in profiles:
        group_matrix_breakdown.append({
            "student_name": p["full_name"],
            "archetype": p["archetype"],
            "strongest_project": p["toughest_project"],
            "strongest_skill": f"Languages: {p['programming_languages']} (AI/ML Exp: {p['ai_ml_experience']})",
            "future_goal": p["future_goal"],
            "leadership_experience": p["club_information"],
            "project_themes": p["project_themes"],
            "intended_major": p["major"],
            "history_assets": p.get("history_assets", [])
        })
    ...
    return {
        ...
        "collaboration_strengths": "High balance across complementary engineering archetypes tracking structural crossovers.",
        ...
        "new_group_analysis_matrix": group_matrix_breakdown,
        "group_history_assets": group_history_assets
    }

# ---------------------------------------------------------------------------
# RESEARCH-BACKED PROMPT ENGINE (OPPORTUNITY DIVERSITY REORIENTATION)
# ---------------------------------------------------------------------------
def generate_master_reasoning_prompt(student_matrix: str, historical_pool: str, group_rules: str) -> str:
    return (
        "You are an expert academic mentor and chief data engineer at BetterMind Labs helping ambitious students discover deep, research-backed problem opportunities.\n"
        ...
        "Projects MUST be generated primarily from Student History + Student Trajectory + Student Strengths + Student Leadership + Hardest Project, NOT from interest areas alone.\n"
        ...
        "Carefully parse the incoming data fields with these exact internal structural weightings:\n"
        "- 30% Weight: toughest_project / history_assets (What advanced architectural exposure or skills have they already demonstrated?)\n"
        "- 20% Weight: future_goal (What is their chosen professional path/long-term objective?)\n"
        "- 15% Weight: club_information (What leadership, community involvement, or extracurricular roles do they hold?)\n"
        "- 15% Weight: existing_project_idea / proposed_idea (What initial topic or seed idea do they want to explore?)\n"
        "- 10% Weight: major / intended_major (What is their intended college major?)\n"
        "- 10% Weight: project_themes (Interest areas are only supporting signals, never the main driver!)\n\n"
        
        "PROJECT DISCOVERY ENGINE AND QUALITY FILTER:\n"
        "1. STRICT NO-GENERIC QUALITY FILTER: Immediately reject generic dashboards, chatbots, basic wrappers, simple trackers, personal productivity tools, study managers, or basic wrappers.\n"
        "2. REQUIRED TECHNICAL COMPLEXITY: Every project must center on non-trivial backend tasks: prediction, optimization, scientific discovery, risk modeling, forecasting, simulation, or research anomaly detection.\n"
        "3. PORTFOLIO-QUALITY TITLE FORMULA: Every title must follow this pattern exactly: '[Memorable Brand Name]: [Specific Research-Oriented Subtitle]'.\n"
        "   Examples: 'IPOIQ: Predicting Long-Term IPO Volatility Through Prospectus Language Analysis', 'TradeCraft: Using Simulation-Based Learning To Improve Teen Investing Decisions', 'SupplyFlow: Forecasting Small-Business Inventory Risk Using Alternative Signals'.\n"
        "   Never use titles like 'AI Dashboard', 'AI Assistant', 'Financial Planner', 'Chatbot', or 'Tracker'.\n\n"
        
        "DATASET QUALITY GATES:\n"
        "Mandate specific, high-fidelity public or professional repositories (e.g., SEC EDGAR, FRED Economic Data, Yahoo Finance Historical Data, Alpha Vantage, NOAA Climate Data, NASA Open Data, MIMIC-IV, PhysioNet, Common Crawl). Never use vague terms like 'Kaggle dataset'. If a Kaggle dataset is used, it must contain the exact title (e.g., 'Kaggle: Credit Card Fraud Detection Dataset').\n\n"
        
        "TEAM WORKFLOW COMPLIANCE GATES (IF APPLICABLE):\n"
        "When processing group configurations, use the provided group_history_assets. Every generated opportunity must reference at least one historical asset from every student in the group. Symmetrically distribute the separate execution sub-tasks so every teammate drives a critical mathematical, infrastructure, or operational segment accounting for at least 20% of pipeline code complexity.\n\n"
        ...
        '      ],\n'
        '      "student_history_connection": "[Detailed technical narrative explicitly connecting this opportunity back to the student\'s history (or the team collective history assets matrix), explaining why this is the natural next chapter of their technical journey, integrating toughest_project milestones, trajectory indicators, and explicit history_assets. If group mode, detail the exact workload split proving why every participant owns at least 20% of engineering operations and which specific history asset of theirs is integrated]",\n'
        '      "target_users": "[Explicit user group or professional persona experiencing the problem]",\n'
        '      "datasets": "[Name of specific, verified high-fidelity data repository, e.g., SEC EDGAR database, FRED Economic Data]",\n'
...
def generate_ideas_external(profile: StudentProfile):
    ...
    try:
        ...
        student_info = {
            ...
            "toughest_project": profile.toughest_project or tally.get("toughest_project", "Advanced engineering prototype build"),
            "proposed_idea": profile.proposed_idea or tally.get("existing_project_idea", "None"),
            "intended_major": tally.get("major", "Computer Science"),
            "future_goal": tally.get("future_goal", "Systems Engineering Frameworks"),
            "project_themes": tally.get("project_themes", []),
            "programming_experience": tally.get("programming_experience", "Intermediate"),
            "ai_ml_experience": tally.get("ai_ml_experience", "Beginner"),
            "club_information": tally.get("club_information", "Local computing and technology building circles"),
            "program_goals": tally.get("program_goals", "Scale non-trivial predictive systems modeling real anomalies"),
            "existing_project_idea": profile.proposed_idea or tally.get("existing_project_idea", "")
        }
        
        if profile.interest_areas:
            manual_themes = [t.strip() for t in profile.interest_areas.split(",") if t.strip()]
            student_info["project_themes"] = list(set([t for t in manual_themes if t in VALID_THEMES] + student_info["project_themes"]))
        ...
        while len(processed_ideas) < 3 and len(processed_ideas) > 0:
            processed_ideas.append(processed_ideas[0])
        ...
        discovery_metadata = DiscoveryMetadata(
            ...
            personalization_summary=f"Personalized heavily utilizing continuous trajectory anchors, history assets, and career intent profiles to unlock next-stage engineering milestones."
        )
...
def generate_ideas_bettermind(request: BettermindGenerateRequest):
    ...
    try:
        ...
        initial_pool = execute_hybrid_retrieval(search_query, group_identity_graph, top_k=2)
        ...
        sys_prompt = generate_master_reasoning_prompt(
            ...
            group_rules=(
                ...
                "Every generated opportunity MUST reference at least one historical asset from group_history_assets for every single student. "
                "Ensure separate execution sub-tasks mapping directly to the explicit recommended_role_split structures. "
                "Every teammate must actively drive a critical mathematical, infrastructure, or operational segment accounting for at least 20% of pipeline code complexity."
            )
        )
        ...
        for item in raw_ideas[:3]:
            generated_opportunity_context = f"{item.get('title')} {item.get('problem_area')} {item.get('project_description')}"
            refined_matched_pool = execute_hybrid_retrieval(generated_opportunity_context, group_identity_graph, top_k=2)
        ...
        discovery_metadata = DiscoveryMetadata(
            ...
            personalization_summary=f"Recommendations were synthesized via cohort trajectory matching layers and combined history assets ({', '.join(group_identity_graph.get('combined_interests', []))})."
        )
...
def niche_down_endpoint(payload: NicheDownRequest):
    ...
    try:
        ...
        while len(processed) < 3 and len(processed) > 0:
