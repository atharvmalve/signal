...
def resolve_and_score_profile(inp: ParticipantInput) -> Dict[str, Any]:
    ...
    # If the student was manually inputted and not matched inside Tally data cache,
    # extract metadata structural clues directly from available text vectors.
    if not tally:
        ...
        # Hydrate secondary evaluation slots via context fallback metrics
...
def compile_identity_graph(profiles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Format multi-participant matrices for precision multi-agent ingestion matching
...
def generate_master_reasoning_prompt(student_matrix: str, historical_pool: str, group_rules: str) -> str:
    return (
        ...
        "MANDATORY INTERNAL STEP 1: STUDENT TRAJECTORY ANALYSIS\n"
        ...
        "- 30% Weight: toughest_project (What advanced architectural exposure or skills have they already demonstrated?)\n"
        ...
        "MANDATORY INTERNAL STEP 2: MULTI-STAGE COGNITIVE GENERATION PIPELINE\n"
        "Execute these stages sequentially within your scratchpad logic before outputting the final response array:\n"
        "STAGE 1: Generate 15 candidate problem statements (strictly granular real-world frictions, NOT product titles yet). For each candidate, internally map: {problem_statement, affected_users, why_unsolved, student_fit_reason}.\n"
        "STAGE 2: Rank all 15 candidates against this specific multi-vector verification matrix:\n"
        "   - 35% Trajectory Fit (Does it logically extend their past work trajectory as the next chapter of their journey?)\n"
        "   - 25% Novelty (Is it clear of basic clichés and typical high school project templates?)\n"
        "   - 20% Feasibility (Can a robust, functional prototype be coded using standard Python infrastructure within 3 weeks?)\n"
        "   - 20% Admissions Value (Does it exhibit deep intellectual curiosity and technical rigor for top-tier universities?)\n"
        "STAGE 3: Select the top 3 highest-scoring concepts and expand them into full structural project models. Always return EXACTLY 3 opportunities.\n\n"
        
        "INTEREST COMBINATION RULE:\n"
        "Intentionally synthesize student historical tech milestones with secondary interests to spawn innovative problem intersections. "
        "For instance, if history shows Robotics and interests point to Finance, look toward inventory-driven micro-forecasting matrices rather than plain stock charts. "
        "If history shows Healthcare ML and interests point to Cybersecurity, target model vulnerability analysis against adversarial noise injections.\n\n"
        ...
        "3. PORTFOLIO-QUALITY TITLE FORMULA: Every title must follow this pattern: '[Memorable Product Name]: [Specific Research-Oriented Subtitle]'.\n"
        "   Examples: 'MissionGuard: Predicting Satellite Component Failures Through Telemetry Analysis', 'IPOIQ: Predicting Long-Term IPO Volatility Using Filing Language'.\n\n"
        
        "TEAM WORKFLOW COMPLIANCE GATES (IF APPLICABLE):\n"
        "When processing group configurations, ensure that individual roles are distributed symmetrically. Evaluate each idea to ensure every single teammate accounts for AT LEAST 20% of the core architecture execution complexity. Avoid strongest-student domination. Synthesize disciplines across student histories seamlessly into the 'team_history_connection' attribute.\n\n"
        
        "HIGH-FIDELITY DATA SOURCES & NO-HALLUCINATION RULES:\n"
        ...
        "Never state that the student built any historical benchmark projects mentioned in the reference dataset pools. Background parameters must come only from the explicitly provided matrix details.\n\n"
        ...
        '      ],\n'
        '      "student_history_connection": "[Detailed technical narrative explicitly connecting this opportunity back to the student\'s history (or the team collective history matrix), explaining why this is the natural next chapter of their technical journey, integrating toughest_project milestones and trajectory indicators. If group mode, detail the exact workload split proving why every participant owns at least 20% of engineering operations]",\n'
...
def generate_ideas_external(profile: StudentProfile):
    ...
    try:
        ...
        sys_prompt = generate_master_reasoning_prompt(
            ...
            group_rules="Individual Student Mode: Prioritize historical milestones, previous hardest engineering challenges, and continuous trajectory milestones."
        )
